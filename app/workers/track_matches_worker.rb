...
class TrackMatchesWorker
  ...
  def perform
    ...
    league_matches = json.reject { |match| match.raw['league_id'].zero? }
    league_match_ids = league_matches.map do |league_match|
      match_id = league_match.raw.fetch('match_id')
      RegisterNewMatchWorker.perform_async(league_match.raw) unless Match.exists?(match_id)
      match_id
    end
